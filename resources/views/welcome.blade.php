@extends('includes.include')
@section('content')



<!-- banner area start -->
<section class="banner__area pt-135 pb-135 bg_img" data-background="assets/images/bg/banner-bg-1.jpg">
    <div class="banner-wrap">
        <div class="container-fluid">
            <div class="row">
                <div class="col-xl-6 col-lg-6 col-md-7">
                    <div class="bannertext">
                        <h5 class="subheading mb-10"><span class="mr-10">//</span>Best IT Solution provider</h5>
                        <h1 class="heading">Advanced <br>Innovative <br>IT Solutions<span>.</span></h1>
                        <p style="letter-spacing: 2px">Let’s create impactful digital products</p>
                        <div class="buttons mt-35">
                            <a href="contact" class="site-btn">Get In Touch <span>+</span></a>
                            <a href="about" class="site-btn transparent">Learn More <span>+</span></a>
                        </div>
                    </div>
                </div>
                {{-- <div class="col-xl-4 col-lg-6 offset-xl-2 col-md-5 mmt-auto">
                        <div class="banner__rightbox">
                            <div class="banner__rightbox--item">
                                <div class="circle">
                                    <input type="text" class="knob" value="0" data-rel="65" data-linecap="round" data-width="75"
                                        data-height="75" data-bgcolor="#DEF5FF" data-fgcolor="#086AD8" data-thickness=".10"
                                        data-readonly="true" disabled>
                                </div>
                                <div class="content">
                                    <h3 class="title">Business Growth</h3>
                                    <p>Level is high</p>
                                </div>
                            </div>
                            <div class="banner__rightbox--item mt-110 ml-30">
                                <div class="circle">
                                    <input type="text" class="knob" value="0" data-rel="53" data-linecap="round" data-width="75"
                                        data-height="75" data-bgcolor="#DEF5FF" data-fgcolor="#086AD8" data-thickness=".10"
                                        data-readonly="true" disabled>
                                </div>
                                <div class="content">
                                    <h3 class="title">Provide Fund</h3>
                                    <p>Don’t worry about it</p>
                                </div>
                            </div>
                        </div>
                    </div> --}}
            </div>
        </div>
    </div>
    <div class="banner-img">
        <img src="assets/images/banner/banner-1.png" alt="">
    </div>
</section>
<!-- banner area end -->


<!-- webmasterpice area start -->
<section class="webmasterpice-area pt-100 pb-100">
    <div class="container">
        <div class="row">
            <div class="col-xl-6">
                <div class="section__heading mb-35">
                    <h4 class="section__heading--title-small"><span class="mr-10">//</span>What We Do Best</h4>
                    <h2 class="section__heading--title">Create Your Own Web
                        Masterpiece<span>.</span></h2>
                    <div class="section__heading--content mt-20">
                        <p>It’s not as hard, or as expensive, as you think. Increase your credibility, attract new customers and
                            position yourself and stay in control of your brand.
                        </p>
                    </div>
                </div>
                <div class="webmasterpice__content mt-35">
                    <p>World is moving so fast and you may be left alone if you have not identified your value. We, as a company would
                        be able to help you to make others understand who you are and your brand by providing a web masterpies that describes who you
                        are. Simple phone call can make a huge impact on your brand!
                    </p>
                    <a href="about" class="site-btn mt-35">Learn More <span>+</span></a>
                </div>
            </div>
            <div class="col-xl-6">
                <div class="row webmasterpice">
                    <div class="col-xl-6 col-lg-6">
                        <div class="webmasterpice__box">
                            <div class="icon">
                                <img src="assets/images/icons/webmasterpice-1.png" alt="">
                            </div>
                            <span>Mobile Friendly
                                Design</span>
                        </div>
                        <div class="webmasterpice__box mt-30">
                            <div class="icon">
                                <img src="assets/images/icons/webmasterpice-2.png" alt="">
                            </div>
                            <span>Web & Cloud Application</span>
                        </div>
                    </div>
                    <div class="col-xl-6 col-lg-6 align-self-center">
                        <div class="webmasterpice__box webmasterpice__box--grey">
                            <div class="icon">
                                <img src="assets/images/icons/webmasterpice-3.png" alt="">
                            </div>
                            <span>Digital Marker
                                Solutions</span>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
<!-- webmasterpice area end -->


<!-- support area start -->
<section class="support__area pt-95 pb-100">
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-xl-7 text-center">
                <div class="section__heading white mb-60">
                    <h4 class="section__heading--title-small"><span class="mr-10">//</span>HIRE US, WHY NOT<span class="ml-10">//</span></h4>
                    <h1 class="section__heading--transparent">Hire Us</h1>
                    <h2 class="section__heading--title">Why Choose Us<span>.</span></h2>
                </div>
            </div>
        </div>
        <div class="row custom-row mt-none-30">
            <div class="col-xl-2 col-lg-4 col-md-4 col-sm-6 mt-30">
                <div class="support__box">
                    <div class="icon">
                        <img src="assets/images/icons/support-icon-1.png" class="default" alt="">
                        <img src="assets/images/icons/support-icon-h-1.png" class="hover" alt="">
                    </div>
                    <div class="content">
                        <h2 class="title mb-15">Our Approach</h2>
                        <p>It's agile. You request, we deliver!</p>
                    </div>
                </div>
            </div>
            <div class="col-xl-2 col-lg-4 col-md-4 col-sm-6 mt-30">
                <div class="support__box">
                    <div class="icon">
                        <img src="assets/images/icons/support-icon-2.png" class="default" alt="">
                        <img src="assets/images/icons/support-icon-h-2.png" class="hover" alt="">
                    </div>
                    <div class="content">
                        <h2 class="title mb-15">Our Values</h2>
                        <p>just YOU! We value our customers.</p>
                    </div>
                </div>
            </div>
            <div class="col-xl-2 col-lg-4 col-md-4 col-sm-6 mt-30">
                <div class="support__box">
                    <div class="icon">
                        <img src="assets/images/icons/support-icon-3.png" class="default" alt="">
                        <img src="assets/images/icons/support-icon-h-3.png" class="hover" alt="">
                    </div>
                    <div class="content">
                        <h2 class="title mb-15">Our Resources</h2>
                        <p>The minds behind the masterpieces.</p>
                    </div>
                </div>
            </div>
            <div class="col-xl-2 col-lg-4 col-md-4 col-sm-6 mt-30">
                <div class="support__box">
                    <div class="icon">
                        <img src="assets/images/icons/support-icon-4.png" class="default" alt="">
                        <img src="assets/images/icons/support-icon-h-4.png" class="hover" alt="">
                    </div>
                    <div class="content">
                        <h2 class="title mb-15">Growth Process</h2>
                        <p>Simply and slowly. We are not in a hurry!</p>
                    </div>
                </div>
            </div>
            <div class="col-xl-2 col-lg-4 col-md-4 col-sm-6 mt-30">
                <div class="support__box">
                    <div class="icon">
                        <img src="assets/images/icons/support-icon-5.png" class="default" alt="">
                        <img src="assets/images/icons/support-icon-h-5.png" class="hover" alt="">
                    </div>
                    <div class="content">
                        <h2 class="title mb-15">Our Support</h2>
                        <p>24/7 on 365 days</p>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
<!-- support area end -->



<!-- about area start -->
<section class="about__area about__area--2 about__area--3 pt-100 pb-140" style="background: #F8F8F8">
    <div class="container">
        <div class="row">
            <div class="col-xl-6">
                <div class="section__heading mb-30 mt-65">
                    <h4 class="section__heading--title-small"><span class="mr-10">//</span>about us</h4>
                    <h2 class="section__heading--title">Who we are...</h2>
                    <div class="section__heading--content mt-20">
                        <p>Neuroon Informatic (Pvt) Ltd is a full-service software development, server hosting and digital marketing agency based in Colombo.</p>
                    </div>
                </div>
                <div class="about__box about__box--2 about__box--3">
                    <ul class="nav" role="tablist">
                        <li class="nav-item"><a class="nav-link active" href="#tab-1" role="tab" data-toggle="tab">Vision</a></li>
                        <li class="nav-item"><a class="nav-link" href="#tab-2" role="tab" data-toggle="tab">Mission</a></li>
                    </ul>
                    <div class="tab-content">
                        <div role="tabpanel" class="tab-pane fade in active show" id="tab-1">
                            <p>Our vision is to provide innovative solution with creative ideas and
                                improve the productivity of Sri Lankan IT sector.
                        </div>
                        <div role="tabpanel" class="tab-pane fade" id="tab-2">
                            <p>The mission of Neuroon Informatic is to serve the Sri Lankan IT sector
                                with high-quality software solutions with a combined of creative ideas.
                                Neuroon Informatic, as a company is mainly focused on augmented
                                reality, machine learning, deep learning, artificially intelligent systems,
                                and cybersecurity. Use software for good purposes, think beyond
                                reality and make what people want, is what Neuroon informatic want.
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-xl-6 text-right">
                <div class="about__bg about__bg--2 about__bg--3">
                    <img src="assets/images/bg/about-bg-2.jpeg" alt="">
                    <div class="call-box">
                        <span>Call Us Anytime!</span>
                        <a href="tel:+94 71 251 5020">+94 71 251 5020</a>
                        <img src="assets/images/icons/call-icon.png" alt="" class="icon">
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
<!-- about area end -->


<!-- cta area start -->
<section class="cta__area pt-95 pb-100 bg_img" data-background="assets/images/bg/cta-bg-1.jpeg" data-overlay="dark" data-opacity="6">
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-xl-6 text-center">
                <div class="section__heading white mb-40">
                    <h4 class="section__heading--title-small"><span class="mr-10">//</span> Call to action<span class="ml-10">//</span></h4>
                    <h2 class="section__heading--title">We Are Waiting For Your
                        Possitive Response<span>.</span></h2>
                </div>
                <div class="cta__btns">
                    <a href="services" class="site-btn">Our Service <span>+</span></a>
                    <a href="about" class="site-btn white transparent">Learn More <span>+</span></a>
                </div>
            </div>
        </div>
    </div>
</section>
<!-- cta area end -->



<!-- service area start -->
<section class="service__area service__area--2 grey-bg pt-95 pb-100">
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-xl-7 text-center">
                <div class="section__heading mb-60">
                    <h4 class="section__heading--title-small"><span class="mr-10">//</span>services<span class="ml-10">//</span></h4>
                    <h1 class="section__heading--transparent white">Services</h1>
                    <h2 class="section__heading--title">Data Analytics, Content & IT <br>
                        Managed Service<span>.</span></h2>
                </div>
            </div>
        </div>
        <div class="row mt-none-30">
            <div class="col-xl-4 col-lg-6 col-md-12 mt-30">
                <div class="service__box service__box--2 service__box--3">
                    <div class="icon">
                        <img src="assets/images/icons/service-d-1.png" class="default" alt="">
                        <img src="assets/images/icons/service-h-1.png" class="hover" alt="">
                    </div>
                    <div class="content">
                        <h2 class="title">IT Services</h2>
                        <p>Handled with experties!</p>
                    </div>
                </div>
            </div>
            <div class="col-xl-4 col-lg-6 col-md-12 mt-30">
                <div class="service__box service__box--2 service__box--3">
                    <div class="icon">
                        <img src="assets/images/icons/service-d-1.png" class="default" alt="">
                        <img src="assets/images/icons/service-h-1.png" class="hover" alt="">
                    </div>
                    <div class="content">
                        <h2 class="title">IT Mangement</h2>
                        <p>Organized and well managed</p>
                    </div>
                </div>
            </div>
            <div class="col-xl-4 col-lg-6 col-md-12 mt-30">
                <div class="service__box service__box--2 service__box--3">
                    <div class="icon">
                        <img src="assets/images/icons/service-d-1.png" class="default" alt="">
                        <img src="assets/images/icons/service-h-1.png" class="hover" alt="">
                    </div>
                    <div class="content">
                        <h2 class="title">Infrastructure Plan</h2>
                        <p>Low cost and high efficient</p>
                    </div>
                </div>
            </div>
            <div class="col-xl-4 col-lg-6 col-md-12 mt-30">
                <div class="service__box service__box--2 service__box--3">
                    <div class="icon">
                        <img src="assets/images/icons/service-d-1.png" class="default" alt="">
                        <img src="assets/images/icons/service-h-1.png" class="hover" alt="">
                    </div>
                    <div class="content">
                        <h2 class="title">Data Security</h2>
                        <p>We know the perfect way!</p>
                    </div>
                </div>
            </div>
            <div class="col-xl-4 col-lg-6 col-md-12 mt-30">
                <div class="service__box service__box--2 service__box--3">
                    <div class="icon">
                        <img src="assets/images/icons/service-d-1.png" class="default" alt="">
                        <img src="assets/images/icons/service-h-1.png" class="hover" alt="">
                    </div>
                    <div class="content">
                        <h2 class="title">Firewel Security</h2>
                        <p>Cutting edge security</p>
                    </div>
                </div>
            </div>
            <div class="col-xl-4 col-lg-6 col-md-12 mt-30">
                <div class="service__box service__box--2 service__box--3">
                    <div class="icon">
                        <img src="assets/images/icons/service-d-1.png" class="default" alt="">
                        <img src="assets/images/icons/service-h-1.png" class="hover" alt="">
                    </div>
                    <div class="content">
                        <h2 class="title">Desktop Cloud</h2>
                        <p>Feel the technology</p>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
<!-- service area end -->




<!-- integration area start -->
<section class="integration__area pt-125 pb-125">
    <div class="container">
        <div class="row">
            <div class="col-xl-12">
                <div class="integration">
                    <div class="row justify-content-center">
                        <div class="col-xl-8 text-center">
                            <div class="section__heading white mb-15">
                                <h4 class="section__heading--title-small"><span class="mr-10">//</span>Integration<span class="ml-10">//</span></h4>
                                <h1 class="section__heading--transparent">Integration</h1>
                                <h2 class="section__heading--title">Seamless Integrations With <br>
                                    Other Analytics Tools<span>.</span></h2>
                            </div>
                            <div class="integration__content">
                                <p>Update your legacy systems with latest technology and enjoy the less pain on maintaining.</p>
                                <div class="integration__btns mt-35">
                                    <a href="services" class="site-btn">Our Service <span>+</span></a>
                                    <a href="about" class="site-btn transparent">Learn More <span>+</span></a>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="integration__icon">
                        <div class="integration__icons--box">
                            <i class="fab fa-java"></i>
                        </div>
                        <div class="integration__icons--box">
                            <i class="fab fa-wordpress"></i>
                        </div>
                        <div class="integration__icons--box">
                            <i class="fab fa-css3"></i>
                        </div>
                        <div class="integration__icons--box">
                            <i class="fab fa-python"></i>
                        </div>
                        <div class="integration__icons--box">
                            <i class="fab fa-dropbox"></i>
                        </div>
                        <div class="integration__icons--box">
                            <i class="fab fa-google"></i>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
<!-- integration area end -->


<!-- why choose area start -->
<section class="whychoose-area pt-95 pb-55">
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-xl-7 text-center">
                <div class="section__heading mb-100">
                    <h4 class="section__heading--title-small"><span class="mr-10">//</span>Hire us, why not<span class="ml-10">//</span></h4>
                    <h1 class="section__heading--transparent">Hire Us</h1>
                    <h2 class="section__heading--title">Why Choose Us</h2>
                </div>
            </div>
        </div>
        <div class="row mt-none-30">
            <div class="col-xl-4 col-lg-6 mt-30">
                <div class="whychoose__box">
                    <div class="thumb-wrap">
                        <div class="thumb">
                            <img src="assets/images/service/wcu-1.jpeg" alt="">
                        </div>
                        <div class="cat">
                            <div class="icon"><i class="fal fa-cogs"></i></div>
                            <h4 class="cat-ttile">Management System</h4>
                        </div>
                    </div>
                    <div class="content text-center mt-25">
                        <div class="cat__inner">
                            <div class="icon"><i class="fal fa-cogs"></i></div>
                            <h4 class="cat-ttile">Management System</h4>
                        </div>
                        <p>We seamlessly merge two key components – economics and information technology. This is the main actual fact.</p>
                        <a href="service-details.html" class="inline-btn">Read More</a>
                    </div>
                </div>
            </div>
            <div class="col-xl-4 col-lg-6 mt-30">
                <div class="whychoose__box">
                    <div class="thumb-wrap">
                        <div class="thumb">
                            <img src="assets/images/service/wcu-2.jpeg" alt="">
                        </div>
                        <div class="cat">
                            <div class="icon"><i class="fal fa-lock"></i></div>
                            <h4 class="cat-ttile">Database Security</h4>
                        </div>
                    </div>
                    <div class="content text-center mt-25">
                        <div class="cat__inner">
                            <div class="icon"><i class="fal fa-lock"></i></div>
                            <h4 class="cat-ttile">Database Security</h4>
                        </div>
                        <p>We seamlessly merge two key components – economics and information technology. This is the main actual fact.</p>
                        <a href="service-details.html" class="inline-btn">Read More</a>
                    </div>
                </div>
            </div>
            <div class="col-xl-4 col-lg-6 mt-30">
                <div class="whychoose__box">
                    <div class="thumb-wrap">
                        <div class="thumb">
                            <img src="assets/images/service/wcu-3.jpeg" alt="">
                        </div>
                        <div class="cat">
                            <div class="icon"><i class="fal fa-globe"></i></div>
                            <h4 class="cat-ttile">Multifuction Technology</h4>
                        </div>
                    </div>
                    <div class="content text-center mt-25">
                        <div class="cat__inner">
                            <div class="icon"><i class="fal fa-globe"></i></div>
                            <h4 class="cat-ttile">Multifuction Technology</h4>
                        </div>
                        <p>We seamlessly merge two key components – economics and information technology. This is the main actual fact.</p>
                        <a href="service-details.html" class="inline-btn">Read More</a>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
<!-- why choose area end -->

<!--  itstaffs area start -->
<section class="itstaffs-area pt-95 pb-95 bg_img" data-background="assets/images/pattern/map-pattern.png">
    <div class="container">
        <div class="row">
            <div class="col-xl-9">
                <div class="section__heading white mb-40">
                    <h4 class="section__heading--title-small">vailable for 8 hours a day!</h4>
                    <h2 class="section__heading--title">Obtaining Further Information
                        By Make A Contact With Our
                        Experienced IT Staffs<span>.</span></h2>
                </div>
                <div class="itstaffs__btns">
                    <a href="about" class="site-btn">Learn More <span>+</span></a>
                    <a href="contact" class="site-btn white transparent">Make A Call <span><i class="fal fa-phone"></i></span></a>
                </div>
            </div>
            {{-- <div class="col-xl-4 offset-xl-1">
                <div class="itstaffs-wrap">
                    <div class="banner__rightbox">
                        <div class="banner__rightbox--item mr-30">
                            <div class="circle">
                                <input type="text" class="knob" value="0" data-rel="65" data-linecap="round" data-width="75"
                                    data-height="75" data-bgcolor="#DEF5FF" data-fgcolor="#086AD8" data-thickness=".10" data-readonly="true"
                                    disabled>
                            </div>
                            <div class="content">
                                <h3 class="title">Business Growth</h3>
                                <p>Level is high</p>
                            </div>
                        </div>
                        <div class="banner__rightbox--item mt-90 ml-60">
                            <div class="circle">
                                <input type="text" class="knob" value="0" data-rel="53" data-linecap="round" data-width="75"
                                    data-height="75" data-bgcolor="#DEF5FF" data-fgcolor="#086AD8" data-thickness=".10" data-readonly="true"
                                    disabled>
                            </div>
                            <div class="content">
                                <h3 class="title">Provide Fund</h3>
                                <p>Don’t worry about it</p>
                            </div>
                        </div>
                    </div>
                </div>
            </div> --}}
        </div>
    </div>
</section>
<!--  itstaffs area end -->

<!-- case study area start -->
<section class="casestudy-area pt-95 pb-100 bg_img" data-background="assets/images/pattern/case-study-pattern.png">

    <div class="container">
        <div class="row justify-content-center">
            <div class="col-xl-7 text-center">
                <div class="section__heading mb-60">
                    <h4 class="section__heading--title-small"><span class="mr-10">//</span>Case Study<span class="ml-10">//</span></h4>
                    <h1 class="section__heading--transparent">Portfolio</h1>
                    <h2 class="section__heading--title">Our Portfolio/Works</h2>
                </div>
            </div>
        </div>

        <div class="row mt-none-30">

            {{-- <div class="col-xl-4 col-lg-6 mt-30">
                    <div class="casestudy__box">
                        <div class="casestudy__box--thumb">
                            <img src="assets/images/service/case-study-1.jpeg" alt="">
                        </div>
                        <div class="casestudy__box--content">
                            <h5 class="sub-title">Software Solutions</h5>
                            <h4 class="title">Kesemiro Software Solution</h4>
                        </div>
                    </div>
                </div>
                <div class="col-xl-4 col-lg-6 mt-30">
                    <div class="casestudy__box">
                        <div class="casestudy__box--thumb">
                            <img src="assets/images/service/case-study-2.jpeg" alt="">
                        </div>
                        <div class="casestudy__box--content">
                            <h5 class="sub-title">App Solutions</h5>
                            <h4 class="title">Kesemiro App Solution</h4>
                        </div>
                    </div>
                </div> --}}


            <div class="swiper mySwiper">


                <div class="swiper-wrapper">
                    <div class="swiper-slide">
                        <div class="col-xl-12 col-lg-6 mt-30">
                            <div class="casestudy__box">
                                <div class="casestudy__box--thumb">
                                    <img src="assets/images/service/case-study-3.jpeg" alt="">
                                </div>
                                <div class="casestudy__box--content">
                                    <h5 class="sub-title">Web Solutions</h5>
                                    <h4 class="title">Kesemiro Web Solution</h4>
                                </div>
                            </div>
                        </div>
                    </div>

                    <div class="swiper-slide">
                        <div class="col-xl-12 col-lg-6 mt-30">
                            <div class="casestudy__box">
                                <div class="casestudy__box--thumb">
                                    <img src="assets/images/service/case-study-3.jpeg" alt="">
                                </div>
                                <div class="casestudy__box--content">
                                    <h5 class="sub-title">Web Solutions</h5>
                                    <h4 class="title">Kesemiro Web Solution</h4>
                                </div>
                            </div>
                        </div>
                    </div>

                    <div class="swiper-slide">
                        <div class="col-xl-12 col-lg-6 mt-30">
                            <div class="casestudy__box">
                                <div class="casestudy__box--thumb">
                                    <img src="assets/images/service/case-study-3.jpeg" alt="">
                                </div>
                                <div class="casestudy__box--content">
                                    <h5 class="sub-title">Web Solutions</h5>
                                    <h4 class="title">Kesemiro Web Solution</h4>
                                </div>
                            </div>
                        </div>
                    </div>


                    <div class="swiper-slide">
                        <div class="col-xl-12 col-lg-6 mt-30">
                            <div class="casestudy__box">
                                <div class="casestudy__box--thumb">
                                    <img src="assets/images/service/case-study-3.jpeg" alt="">
                                </div>
                                <div class="casestudy__box--content">
                                    <h5 class="sub-title">Web Solutions</h5>
                                    <h4 class="title">Kesemiro Web Solution</h4>
                                </div>
                            </div>
                        </div>
                    </div>


                    <div class="swiper-slide">
                        <div class="col-xl-12 col-lg-6 mt-30">
                            <div class="casestudy__box">
                                <div class="casestudy__box--thumb">
                                    <img src="assets/images/service/case-study-3.jpeg" alt="">
                                </div>
                                <div class="casestudy__box--content">
                                    <h5 class="sub-title">Web Solutions</h5>
                                    <h4 class="title">Kesemiro Web Solution</h4>
                                </div>
                            </div>
                        </div>
                    </div>

                    <div class="swiper-slide">
                        <div class="col-xl-12 col-lg-6 mt-30">
                            <div class="casestudy__box">
                                <div class="casestudy__box--thumb">
                                    <img src="assets/images/service/case-study-3.jpeg" alt="">
                                </div>
                                <div class="casestudy__box--content">
                                    <h5 class="sub-title">Web Solutions</h5>
                                    <h4 class="title">Kesemiro Web Solution</h4>
                                </div>
                            </div>
                        </div>
                    </div>



                    <div class="swiper-slide">
                        <div class="col-xl-12 col-lg-6 mt-30">
                            <div class="casestudy__box">
                                <div class="casestudy__box--thumb">
                                    <img src="assets/images/service/case-study-3.jpeg" alt="">
                                </div>
                                <div class="casestudy__box--content">
                                    <h5 class="sub-title">Web Solutions</h5>
                                    <h4 class="title">Kesemiro Web Solution</h4>
                                </div>
                            </div>
                        </div>
                    </div>



                </div>
                {{-- <div class="swiper-button-next"></div> --}}
                {{-- <div class="swiper-button-prev"></div> --}}
                <div class="swiper-pagination" style="position: absolute;
                    bottom: -6px;"></div>
            </div>


        </div>






        <div class="itstaffs__btns__works">
            <a href="portfolio" class="site-btn">Learn More <span>+</span></a>
        </div>


    </div>
</section>
<!-- case study area end -->

<!-- news-feed area start -->
<section class="news-feed-area grey-bg pt-100 pb-100">
    <div class="container">
        <div class="row">
            <div class="col-xl-6">
                <div class="testimonials owl-carousel">
                    <div class="testimonial">
                        <h4 class="title mb-20">Great Quality Software</h4>
                        <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore to magna
                            aliqua. Ut enim ad minim veniam, quis nostrud a exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.
                            Duis aute irure dolor in reprehenderit in the voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur
                            sint occaecat cupidatat non proident.</p>
                        <div class="authore mt-25 d-flex align-items-center">
                            <div class="authore--thumb mr-20">
                                <img src="assets/images/others/testimonial-authore-1.png" alt="">
                            </div>
                            <div class="authore--content">
                                <h5 class="name">Rosalina D. William</h5>
                                <span class="designatin">Founder, Romada Co.</span>
                            </div>
                        </div>
                        <img src="assets/images/icons/quote.png" alt="" class="quote-icon">
                    </div>
                    <div class="testimonial">
                        <h4 class="title mb-20">Great Quality Software</h4>
                        <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore to magna
                            aliqua. Ut enim ad minim veniam, quis nostrud a exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.
                            Duis aute irure dolor in reprehenderit in the voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur
                            sint occaecat cupidatat non proident.</p>
                        <div class="authore mt-25 d-flex align-items-center">
                            <div class="authore--thumb mr-20">
                                <img src="assets/images/others/testimonial-authore-1.png" alt="">
                            </div>
                            <div class="authore--content">
                                <h5 class="name">Rosalina D. William</h5>
                                <span class="designatin">Founder, Romada Co.</span>
                            </div>
                        </div>
                        <img src="assets/images/icons/quote.png" alt="" class="quote-icon">
                    </div>
                    <div class="testimonial">
                        <h4 class="title mb-20">Great Quality Software</h4>
                        <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore to magna
                            aliqua. Ut enim ad minim veniam, quis nostrud a exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.
                            Duis aute irure dolor in reprehenderit in the voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur
                            sint occaecat cupidatat non proident.</p>
                        <div class="authore mt-25 d-flex align-items-center">
                            <div class="authore--thumb mr-20">
                                <img src="assets/images/others/testimonial-authore-1.png" alt="">
                            </div>
                            <div class="authore--content">
                                <h5 class="name">Rosalina D. William</h5>
                                <span class="designatin">Founder, Romada Co.</span>
                            </div>
                        </div>
                        <img src="assets/images/icons/quote.png" alt="" class="quote-icon">
                    </div>
                    <div class="testimonial">
                        <h4 class="title mb-20">Great Quality Software</h4>
                        <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore to magna
                            aliqua. Ut enim ad minim veniam, quis nostrud a exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.
                            Duis aute irure dolor in reprehenderit in the voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur
                            sint occaecat cupidatat non proident.</p>
                        <div class="authore mt-25 d-flex align-items-center">
                            <div class="authore--thumb mr-20">
                                <img src="assets/images/others/testimonial-authore-1.png" alt="">
                            </div>
                            <div class="testimonial__authore--content">
                                <h5 class="name">Rosalina D. William</h5>
                                <span class="designatin">Founder, Romada Co.</span>
                            </div>
                        </div>
                        <img src="assets/images/icons/quote.png" alt="" class="quote-icon">
                    </div>
                </div>
            </div>
            <div class="col-xl-6">
                <div class="newsfeedwrap">
                    <div class="section__heading mb-30">
                        <h4 class="section__heading--title-small"> <span class="mr-10">//</span>news Feeds</h4>
                        <h2 class="section__heading--title">Get Every Single <br>
                            Update Here<span>.</span></h2>
                    </div>
                    <div class="row">
                        <div class="col-xl-12">
                            <div class="news__items mt-none-30">
                                <div class="news__item mt-30">
                                    <div class="thumb">
                                        <img src="assets/images/news/news-small-1.jpeg" alt="">
                                    </div>
                                    <div class="content">
                                        <ul class="meta">
                                            <li><a href="#0"><i class="fa fa-calendar-alt"></i> 24th June 2020</a></li>
                                            <li><a href="#0"><i class="fa fa-user"></i> By Admin</a></li>
                                        </ul>
                                        <h4 class="title"><a href="news-details.html">We are specialists in both economics
                                                and information technologies.</a></h4>
                                    </div>
                                </div>
                                <div class="news__item mt-30">
                                    <div class="thumb">
                                        <img src="assets/images/news/news-small-2.jpeg" alt="">
                                    </div>
                                    <div class="content">
                                        <ul class="meta">
                                            <li><a href="#0"><i class="fa fa-calendar-alt"></i> 24th June 2020</a></li>
                                            <li><a href="#0"><i class="fa fa-user"></i> By Admin</a></li>
                                        </ul>
                                        <h4 class="title"><a href="news-details.html">We are specialists in both economics
                                                and information technologies.</a></h4>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
<!-- news-feed area end -->



@endsection